...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        /* v3 card: white ground, one ink hairline, 18px radius, and the
         * two-layer shadow (tight contact + wide soft). Not a ring — the
         * border participates in layout so inner tiles line up. */
        "group/card border-border bg-card text-card-foreground shadow-card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl border py-(--card-spacing) text-sm/relaxed [--card-spacing:--spacing(5)] has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(4)] *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
        className,
      )}
      {...props}
    />
